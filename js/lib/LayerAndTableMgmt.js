...
define([
    ...
    return declare([], {
        ...
        //EGE: update status of primary feature service
        updateMainFeatureLayer: function (status,item) {
            // Amend a copy of the comment with the foreign key pointing to the
            // associated item

            BizStat = status.toUpperCase();
            

            if (BizStat == "OPEN"){
                item.attributes.reports_Open = item.attributes.reports_Open + 1;
                item.attributes.Open_Closed = "Open";
            }
            else if (BizStat == "CLOSED"){
                item.attributes.reports_Closed = item.attributes.reports_Closed + 1;
                item.attributes.Open_Closed = "Closed";
            }
            else{
                console.log('error with setting the business status data');
            }
            

            // update the main item Layer
        ...
        /**
         * Increments the designated "Open_Closed" field for the specified item.
